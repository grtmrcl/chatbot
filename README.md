...
| `PAIFU_DIR` | 牌譜 JSON ファイルの保存先ディレクトリ（牌譜インポート機能を使う場合は必須） |
| `AMAE_KOROMO_SCRIPTS_DIR` | amae-koromo-scripts のディレクトリパス（牌譜インポート機能を使う場合は必須） |
...
| `paifu_import` | `true` を設定すると牌譜インポート機能を有効化する。省略・`false` の場合は無効 |
...
bot にメンションしつつ `.json` ファイルを添付すると、牌譜をインポートする。

```
@bot ファイルを添付
```

- `DISCORD_SERVERS` の該当チャンネル（またはサーバー）設定に `"paifu_import": true` がある場合のみ有効
- 添付ファイルを `PAIFU_DIR` に保存後、`AMAE_KOROMO_SCRIPTS_DIR` で `docker compose run` を実行する
- 複数ファイルの同時添付も可能
- `PAIFU_DIR` / `AMAE_KOROMO_SCRIPTS_DIR` が未設定の場合はエラーを返す
...
- 登録先の列1〜4に、イベント名・開始日（yyyy/mm/dd）・終了日（yyyy/mm/dd）・手動登録フラグ（`TRUE`）を書き込む
- イベント名が重複する場合は上書き登録する
- 手動登録フラグが付いた行は `get_events.py` バッチ実行時に削除されず保持される

#### イベントリマインダー

翌日または3日後に終了するイベントを通知する。「終了日」列を参照する。
